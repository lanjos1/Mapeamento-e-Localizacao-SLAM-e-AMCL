...
# 1. Verificar a existência da bag do Gmapping
print("🔄 Carregando a rosbag do GMAPPING via ROS...")
nome_bag = 'resultados_amcl_gmapping.bag'
...
print("\n" + "="*50)
print("📊 RESULTADOS CIENTÍFICOS FIXOS: GMAPPING + AMCL")
print("="*50)
...
plt.subplot(1, 2, 1)
plt.plot(tempo, erros_posicao, color='#e65100', linewidth=1.5, label='Erro Absoluto Calibrado')
plt.axhline(y=rmse_posicao, color='black', linestyle='--', label=f'RMSE ({rmse_posicao:.3f}m)')
plt.title('Estabilidade da Localização (Gmapping - URDF Offset)')
...
plt.subplot(1, 2, 2)
plt.plot(tempo, erros_orientacao, color='#006064', linewidth=1.5, label='Erro Angular Absoluto')
plt.axhline(y=np.mean(erros_orientacao), color='black', linestyle='--', label=f'Média ({np.mean(erros_orientacao):.2f}°)')
plt.title('Erro de Orientação Absoluta (Gmapping - Yaw)')
plt.xlabel('Tempo (segundos)')
...
plt.tight_layout()
nome_grafico = 'analise_gmapping_amcl_fixo.png'
