...
def processar_bag(nome_bag, label_slam):
    ...
    if not os.path.exists(nome_bag):
        print(f" Arquivo '{nome_bag}' não encontrado. Proximo...")
        return None
...
if res_hector and res_gmapping:
    ...
    # Plot Posição
    plt.subplot(1, 2, 1)
    plt.plot(res_hector['tempo'], res_hector['erros_pos'], color='#b71c1c', label=f"Hector SLAM (RMSE: {res_hector['rmse_pos']:.3f}m)", alpha=0.8)
    plt.plot(res_gmapping['tempo'], res_gmapping['erros_pos'], color='#e65100', label=f"Gmapping (RMSE: {res_gmapping['rmse_pos']:.3f}m)", alpha=0.8)
    plt.title('Estabilidade do Erro de Posição')
    ...
    # Plot Orientação
    plt.subplot(1, 2, 2)
    plt.plot(res_hector['tempo'], res_hector['erros_ori'], color='#0d47a1', label=f"Hector SLAM (RMSE: {res_hector['rmse_ori']:.2f}°)", alpha=0.8)
    plt.plot(res_gmapping['tempo'], res_gmapping['erros_ori'], color='#004d40', label=f"Gmapping (RMSE: {res_gmapping['rmse_ori']:.2f}°)", alpha=0.8)
    plt.title('Erro Angular (Yaw)')
    plt.xlabel('Tempo (segundos)')
    ...
    plt.tight_layout()
    nome_grafico = 'analise_comparativa_amcl.png'
